...
| **2 · Dev-owned CLI**                      | Scaffolded: `<name>/workflows/<wf>.ts` + `<name>/mycli.ts` (from `bun create @bastani/atomic-cli --template=standalone-cli`). Legacy `src/`-rooted: `<repo>/src/workflows/<name>/<agent>.ts` + `<repo>/src/<agent>-worker.ts` (or `src/cli.ts` for multi-workflow registries) | `bun run mycli.ts --<flag>=<value>` (scaffolded) or `bun run src/<agent>-worker.ts --<flag>=<value>` (legacy) | The user is building their *own* CLI surface, an internal tool, an `examples/` reference, or anything where the workflow is not meant to be discoverable by the wider `atomic` CLI |
