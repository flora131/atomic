/**
 * Design-Driven workflow for GitHub Copilot — a generalized "Coding Backwards"
 * frontend implementation pipeline.
 *
 * Mirrors the Claude variant but adapted for the Copilot SDK's session+tool
 * model: `session.send` / `session.sendAndWait`, tool-based structured output
 * (via `defineTool` with Zod parameters), and agent-as-stage-config rather
 * than inline `@agent` syntax.
 *
 ...
 *                              (TODO: add a --interactive flag that shows the user
 *                               the playwright views if they want to see how each
 ...
 *
 * Run: atomic workflow -n coding-backwards-design -a copilot "<your spec>"
 */
...
const SUBMIT_COMPONENT_PLAN_DESCRIPTION =
  "Submit the structured component implementation plan. You MUST call this " +
  "tool exactly once with your complete plan. Do not output the plan as " +
  "plain text — use this tool.";

// ── Component plan structured output for parallel builds ─────────
// Copilot's defineTool requires Zod parameters (it has no native
// `json_schema` output mode like Claude), so the schema lives here
// and is re-used for the tool definition + the resulting type.
    ...
    const discovery = await ctx.stage(
      ...
      async (s) => {
        await s.session.send({
          prompt: `You are an expert design analyst. Your job is to produce a COMPLETE design brief by analyzing the provided references.
...
- If a reference is inaccessible, document what you can observe and note the gap.`,
    ...
    const critique = await ctx.stage(
      ...
      async (s) => {
        ...
        await s.session.send({
          prompt: `Read the design brief at ${discoveryTranscript.path}.
...
CRITICAL: If the design involves displaying images or artwork from references, the critique must make clear that ALL such content uses real image URLs/sources, never colored placeholder divs. Real content is the hero — the design must frame and enhance it.`,
    ...
    const architecturePlan = await ctx.stage(
      ...
      async (s) => {
        ...
        await s.session.send({
          prompt: `Read the design brief at ${discoveryTranscript.path} and the critique at ${critiqueTranscript.path}.
...
Choose what fits. The README should justify the choice.`,
    ...
    const scaffold = await ctx.stage(
      ...
      async (s) => {
        ...
        await s.session.send({
          prompt: `Read the DESIGN.md referenced in ${planTranscript.path} and the design brief at ${discoveryTranscript.path}.
...
- The file structure matches DESIGN.md`,
    ...
    let componentPlanCapture: ComponentPlan | null = null;
    const componentPlanTool = defineTool("submit_component_plan", {
      description: SUBMIT_COMPONENT_PLAN_DESCRIPTION,
      parameters: ComponentPlanSchema,
      skipPermission: true,
      handler: async (data: ComponentPlan) => {
        componentPlanCapture = data;
        return "Component plan submitted successfully.";
      },
    });
    ...
    const componentPlan = await ctx.stage(
      ...
      {},
      { tools: [componentPlanTool] },
      async (s) => {
        const scaffoldTranscript = await s.transcript(scaffold);
        await s.session.sendAndWait({
          prompt: `Read the DESIGN.md in the project root and the scaffold transcript at ${scaffoldTranscript.path}.
...
You MUST submit your plan via the \`submit_component_plan\` tool — do not output the plan as plain text.`,
        });
        s.save(await s.session.getMessages());
        return componentPlanCapture;
      },
    );
      ...
      await Promise.all(
        components.map((component) =>
          ctx.stage(
            ...
            async (s) => {
              await s.session.send({
                prompt: `Read DESIGN.md and the relevant source files: ${component.files.join(", ")}.
      ...
      await ctx.stage(
        ...
        async (s) => {
          const componentNames = components
            ...
            .join(", ");
          await s.session.send({
            prompt: `You have access to the \`/playwright-cli\` skill for browser automation.
...
CRITICAL: Do NOT introduce new bugs while fixing existing ones. If a fix changes behavior in one area, verify that other areas still work correctly.`,
      ...
      // Visual-analysis structured-output capture (tool-based, since
      // Copilot doesn't support a native json_schema output mode).
      let visualCapture: ReviewResult | null = null;
      const visualReviewTool = defineTool("submit_review", {
        description: SUBMIT_REVIEW_DESCRIPTION,
        parameters: ReviewResultSchema,
        skipPermission: true,
        handler: async (data: ReviewResult) => {
          visualCapture = filterActionable(data);
      ...
      const [locatorResult, analyzerResult, patternResult, visualResult] =
        await Promise.all([
          ctx.stage(
            ...
            async (s) => {
              await s.session.send({
                prompt: discoveryPrompts.locator,
              });
          ...
          ctx.stage(
            ...
            async (s) => {
              await s.session.send({
                prompt: discoveryPrompts.analyzer,
              });
              const messages = await s.session.getMessages();
              s.save(messages);
              return getAssistantText(messages);
          ...
          ctx.stage(
            ...
            async (s) => {
              await s.session.send({
                prompt: discoveryPrompts.patternFinder,
              });
              const messages = await s.session.getMessages();
              s.save(messages);
              return getAssistantText(messages);
          ...
          ctx.stage(
            ...
            {},
            { tools: [visualReviewTool] },
            async (s) => {
              await s.session.sendAndWait({
                prompt: `You are a visual QA analyst. Your job is to verify the rendered frontend against its design specification and produce structured review findings.

You have access to the \`/playwright-cli\` skill for browser automation, and a \`submit_review\` tool for submitting your findings.
...
Submit your findings via the \`submit_review\` tool. Each finding must include:
...
When finished, STOP the dev server.`,
    ...
    await ctx.stage(
      ...
      async (s) => {
        await s.session.send({
          prompt: `Start the dev server: ${devCommand}
