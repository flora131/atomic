...
# Extract feature_list_path with default fallback
FEATURE_LIST_PATH=$(echo "$FRONTMATTER" | grep '^feature_list_path:' | sed 's/feature_list_path: *//')
...
test_all_features_passing() {
    ...
    if [[ "$total_features" -eq 0 ]]; then
        echo "ERROR: research/feature-list.json is empty." >&2
        return 1
    fi
