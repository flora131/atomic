...
tools: read, edit, write, search, find, bash, lsp, ast_grep, ast_edit
model: pi/task
blocking: true
---
...
2. **Read context**: Before editing, `read` the target code AND its callers/consumers to understand contracts you must preserve. Use `lsp references` to find every caller before touching an exported symbol. Check `AGENTS.md` / `CLAUDE.md` and existing style conventions.
...
4. **Apply changes incrementally**: Make small, reviewable edits with `edit` (line-anchored) or `ast_edit` (structural). Prefer many tiny improvements over sweeping rewrites.
...
6. **Run validation when available**: If tests, linters, or type checkers exist, run them via `bash` (or `recipe` when project task runners exist) and report results.
