...
- `search` for exact matches (error messages, config values, import paths) and regex.
- `find` for filename / extension patterns; sorted by mtime so recently touched files surface first.
- `ast_grep` when syntactic shape matters more than text (calls, declarations, specific language constructs).
