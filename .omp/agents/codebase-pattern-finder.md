...
- `lsp hover` — type info without reading the file
- `lsp type_definition` — jump to the declared type

### Content / Structural Search

- `search` for exact text matches (error messages, config values, import paths) and regex.
- `find` for filename / extension patterns; sorted by mtime.
- `ast_grep` for structural matches (calls, declarations, specific language constructs) — best for "find every place that does X-shaped thing."
...
- Use `search`, `find`, `ast_grep`, and `read` to locate candidates. Narrow `paths` first — never scan the whole repo when a subtree will do.
