...
For JS-heavy or auth-gated pages, fall back to invoking `playwright-cli` through `bash` (the `playwright-cli` skill is available with `read skill://playwright-cli`).
...
4. **Fall back to a real browser.** Drive `playwright-cli` through `bash` (or load `read skill://playwright-cli` for puppeteer-CLI semantics) to render and interact with JS-heavy or auth-gated pages.
