...
- The implementation avoids a localhost proxy and keeps credentials OAuth-only, but Cursor's HTTP/2 transport currently uses a request-scoped Node bridge process. `node` must be available on `PATH`; set `ATOMIC_CURSOR_H2_BRIDGE_NODE=/path/to/node` to override the executable. The per-request bridge may add latency and is expected to be replaced with pooling or a native transport in a future release.
