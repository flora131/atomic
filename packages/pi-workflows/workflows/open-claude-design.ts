...
export default defineWorkflow("open-claude-design")
  ...
  .run(async (ctx) => {
    ...
    const onboarding = await ctx.stage("onboarding").prompt(
    ...
    const importResult = await ctx.stage("import").prompt(
    ...
    const generated = await ctx.stage("generate").prompt(
    ...
    const refined = await ctx.stage("refine").prompt(
    ...
    const handoff = await ctx.stage("export-handoff").complete(
