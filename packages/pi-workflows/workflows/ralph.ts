...
export default defineWorkflow("ralph")
  ...
  .run(async (ctx) => {
    ...
    let plan = await ctx.stage("plan").prompt(
    ...
    for (let iteration = 1; iteration <= cap; iteration++) {
      ...
      const result = await ctx.stage(`orchestrate-${iteration}`).prompt(
      ...
      const review = await ctx.stage(`review-${iteration}`).complete(
      ...
      plan = await ctx.stage(`replan-${iteration}`).complete(
    ...
    return {
      ...
      plan,
      approved,
      iterations_completed: Math.min(cap, cap), // actual count tracked by loop
    };
  })
  .compile();
