...
Ordinary children without `outputSchema` can still use the generic default `structured_output` tool as a final machine-readable answer. When such a child ends with a successful `structured_output` tool result and no assistant follow-up, the tool-result JSON text becomes the child's `finalOutput` and chain handoff text; it does not set `result.structuredOutput`, which remains reserved for schema-backed validated captures.
