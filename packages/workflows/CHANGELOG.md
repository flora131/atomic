...
- Fixed mouse select-to-copy and text highlighting inside the workflow session chat node (attached stage chat composer/transcript and interactive stage prompts) so they match the main chat UI. The stage chat surfaces no longer put the terminal into application mouse-reporting mode (`\x1b[?1000h`), which had suppressed the terminal's native click-drag text selection; transcript scrolling stays at parity with main chat through the shared keyboard bindings (PageUp/PageDown/Home/End). Mouse-reporting sequences for the graph view now route through pi-tui's `Terminal.write` builtin instead of a bare `process.stdout.write` for more robust cross-platform/terminal behavior ([#1110](https://github.com/flora131/atomic/issues/1110)).
