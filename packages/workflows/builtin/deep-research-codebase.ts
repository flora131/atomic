...
const CODEBASE_SKILLS = {
  locator: "codebase-locator — use this skill's search-first discipline when mapping where files, symbols, docs, tests, and configuration live.",
  analyzer: "codebase-analyzer — use this skill's evidence-driven deep-read style when explaining behavior, architecture, control flow, data flow, and edge cases.",
  patternFinder: "codebase-pattern-finder — use this skill's example-mining approach when separating reusable conventions from one-off details.",
  researchLocator: "codebase-research-locator — use this skill's historical-discovery approach when finding prior research, specs, ADRs, issues, and TODOs.",
  researchAnalyzer: "codebase-research-analyzer — use this skill's synthesis approach when extracting decisions, constraints, stale assumptions, and open questions from prior research.",
  onlineResearcher: "codebase-online-researcher — use this skill's source-citing approach when external documentation or ecosystem behavior materially affects the answer.",
} as const;

function codebaseSkillGuidance(...skills: readonly (keyof typeof CODEBASE_SKILLS)[]): string {
...
function countCodebaseLines(): number {
  try {
    ...
    const files = gitFiles.success && gitFiles.stdout
      ? gitFiles.stdout.toString().split("\n").filter((line) => line.length > 0)
      : [];
    ...
    for (let i = 0; i < files.length; i += batchSize) {
      ...
      for (const line of wc.stdout.toString().split("\n")) {
        ...
        if (countText === undefined || path === undefined || path === "total") continue;
...
function calculatePartitionCap(requestedMax: number, codebaseLines: number): number {
  if (!Number.isFinite(codebaseLines) || codebaseLines <= 0) return requestedMax;
  return Math.max(1, Math.min(requestedMax, Math.ceil(codebaseLines / LOC_PER_PARTITION)));
}
...
function findResult(results: readonly WorkflowTaskResult[], name: string): WorkflowTaskResult | undefined {
  return results.find((result) => result.name === name || result.stageName === name);
}
...
function specialistSummary(
  ...
  return partitions
    .map((partition, index) => {
      const i = index + 1;
      const locator = findResult(wave1, `locator-${i}`)?.text ?? "(no locator output)";
      const patterns = findResult(wave1, `pattern-finder-${i}`)?.text ?? "(no pattern output)";
      const analyzer = findResult(wave2, `analyzer-${i}`)?.text ?? "(no analyzer output)";
      const online = findResult(wave2, `online-researcher-${i}`)?.text ?? "(no online research output)";
      return [
...
export default defineWorkflow("deep-research-codebase")
  ...
  .input("max_partitions", {
    ...
    description: "Maximum number of codebase partitions to explore in parallel. Actual partitions scale by one per 10K LoC, capped by this value.",
  })
  .run(async (ctx) => {
    ...
    const initialDiscovery = await ctx.parallel([
      {
        name: "codebase-scout",
        task: taggedPrompt([
          ["role", "You are a senior codebase research scout preparing work for specialist agents."],
          ["objective", `Map the repository. Research question: ${prompt}`],
          ["codebase_skills", codebaseSkillGuidance("locator", "analyzer", "patternFinder")],
          ["instructions", [
      ...
      },
      {
        ...
        task: taggedPrompt([
          ...
          ["objective", "Find existing docs, specs, ADRs, issues/PR notes, TODOs, and research artifacts relevant to the task."],
          ["task", "{task}"],
          ...
          ["output_format", "A markdown table with columns: Path, Evidence, Relevance, Confidence."],
        ]),
        ...explorerModelConfig,
      },
    ], { task: prompt });

    const scout = findResult(initialDiscovery, "codebase-scout") ?? initialDiscovery[0]!;
    const historyLocator = findResult(initialDiscovery, "history-locator") ?? initialDiscovery[1]!;
    const history = await ctx.chain([
      {
        name: "history-analyzer",
        task: taggedPrompt([
          ["role", "You synthesize prior project research for downstream investigators."],
          ["objective", `Extract reusable historical context. Research question: ${prompt}`],
    ...
    const partitionPlan = await ctx.task("partition", {
      prompt: taggedPrompt([
        ...
        ["objective", `Return at most ${partitionCap} independent partitions for this research question: ${prompt}`],
    ...
    const wave1Steps: WorkflowTaskStep[] = partitions.flatMap((partition, index) => {
      const i = index + 1;
      return [
        {
          name: `locator-${i}`,
    ...
    const wave2Steps: WorkflowTaskStep[] = partitions.flatMap((partition, index) => {
      const i = index + 1;
      ...
      return [
        ...
        },
        {
          ...
          task: taggedPrompt([
            ["role", "You are an ecosystem and documentation research specialist."],
            ["assignment", `Partition ${i}/${partitions.length}: ${partition}`],
            ["research_question", prompt],
    ...
    const aggregate = await ctx.task("aggregator", {
      prompt: taggedPrompt([
        ...
        ["prior_research_overview", historyOverview || "(no prior research found)"],
        ["specialist_reports", specialistReports],
        ["codebase_skills", codebaseSkillGuidance("analyzer", "researchAnalyzer", "onlineResearcher")],
        ["instructions", [
