...
function reviewApproved(text: string): boolean {
  ...
  if (normalized.includes("patch is correct") || normalized.includes("overall_correctness: patch is correct")) {
    return true;
  }
  if (normalized.startsWith("approved") || normalized.includes("no actionable findings")) {
    return true;
  }
  return false;
}
...
export default defineWorkflow("ralph")
  .description("Plan → orchestrate → simplify → parallel review loop with bounded iteration.")
  .input("prompt", {
    type: "text",
    required: true,
  ...
  .run(async (ctx) => {
    ...
    let orchestratorModelConfig = {
      model: "openai/gpt-5.5",
      fallbackModels: ["github-copilot/gpt-5.5", "anthropic/claude-sonnet-4-6", "github-copilot/claude-sonnet-4.6"],
    ...
    for (let iteration = 1; iteration <= maxLoops; iteration += 1) {
      ...
      const planAndExecute = await ctx.chain([
        {
          ...
          task: taggedPrompt([
            ["role", "You are a technical architect. Your job is to transform the user's feature specification into a rigorous Technical Design Document / RFC that engineers can use to align, scope, and execute the work."],
            ...
            ["previous_review_findings", reviewReport ? "Previous review findings:\n{previous}" : "No prior review findings; this is the first iteration."],
            ["short_circuit", [
          ...
          ...(reviewReport ? { previous: { name: "review-report", text: reviewReport } } : {}),
          ...plannerModelConfig
        },
        {
          name: `orchestrator-${iteration}`,
          task: taggedPrompt([
            ["role", "You are a sub-agent orchestrator with many tools available. Your primary implementation tool is the `subagent` tool."],
            ["objective", `Implement iteration ${iteration}/${maxLoops} for the task: ${prompt}`],
      ...
      const simplifier = await ctx.task(`code-simplifier-${iteration}`, {
        prompt: taggedPrompt([
          ...
          ["objective", `Refine recently modified code for this task while preserving exact behavior: ${prompt}`],
      ...
      const discovery = await ctx.parallel([
        {
          ...
          task: taggedPrompt([
            ["role", "You locate project infrastructure needed for patch review."],
            ["objective", `Find review-relevant infrastructure for the task: ${prompt}`],
            ["instructions", [
            ...
            ["output_format", "Markdown table: Area | Path/command | Why it matters | Confidence."],
          ]),
          ...explorerModelConfig
        },
        {
          ...
          task: taggedPrompt([
            ...
            ["objective", `Assess infrastructure and changed-code risks for the task: ${prompt}`],
            ["instructions", [
            ...
            ["output_format", "Markdown with sections: Confirmed risks, Speculative risks, Validation commands, Evidence."],
          ]),
          ...explorerModelConfig
        },
        {
          ...
          task: taggedPrompt([
            ...
            ["objective", `Extract conventions relevant to reviewing this task: ${prompt}`],
            ["instructions", [
            ...
            ["output_format", "Markdown with sections: Required conventions, Useful examples, Exceptions, Review implications."],
          ]),
          ...explorerModelConfig
        },
      ], { task: prompt });
      ...
      const reviewPrompt = taggedPrompt([
        ["role", "You are acting as a reviewer for a proposed code change made by another engineer."],
        ["objective", `Review iteration ${iteration}/${maxLoops} for the task: ${prompt}`],
      ...
      approved = reviews.length > 0 && reviews.every((review) => reviewApproved(review.text));
