 ...
 *
 * Responsibilities:
 *  1. Inject <APP>_WORKFLOW_RUN_ID / <APP>_WORKFLOW_STAGE_ID env vars so
 *     delegated child sessions carry workflow context.
...
export const WORKFLOW_RUN_ID_ENV = `${ENV_PREFIX}_WORKFLOW_RUN_ID`;
export const WORKFLOW_STAGE_ID_ENV = `${ENV_PREFIX}_WORKFLOW_STAGE_ID`;
...
// ---------------------------------------------------------------------------
// Env injection — <APP>_WORKFLOW_RUN_ID / <APP>_WORKFLOW_STAGE_ID
// ---------------------------------------------------------------------------

export type WorkflowEnvVars = Record<string, string>;
...
export function injectWorkflowEnv(runId: string, stageId: string): WorkflowEnvVars {
  return {
    [WORKFLOW_RUN_ID_ENV]: runId,
    [WORKFLOW_STAGE_ID_ENV]: stageId,
  };
}
 ...
 */
export function readWorkflowEnv(): { runId?: string; stageId?: string } {
  return {
    runId: process.env[WORKFLOW_RUN_ID_ENV],
    stageId: process.env[WORKFLOW_STAGE_ID_ENV],
  };
}
