...
export interface StreamMessageOptions {
  /** Sub-agent/agent name for provider-specific dispatch metadata (OpenCode AgentPartInput, Claude query agent option). */
  agent?: string;
  ...
  /** Whether the transcript content for this run should be rendered. */
  visibility?: StreamRunVisibility;
  /** Runtime kind used by the run registry. */
  runKind?: StreamRunKind;
  /** Optional parent run when a command/workflow creates a child stream. */
  parentRunId?: string;
}
...
export interface CommandContext {
  ...
  /**
   * Start a run-backed assistant stream and receive a handle for completion.
   * This is the preferred imperative streaming entrypoint for workflows.
   */
...
export interface CommandResult {
  ...
  mcpServers?: import("@/services/agents/types.ts").McpServerConfig[];
