/**
 * Companion-package detection for `/workflows-doctor`.
 *
 * Why this exists:
 *   `@bastani/atomic-workflows` orchestrates a few first-party pi packages
 *   at runtime (delegation, MCP access, HIL bridge, web fetch). They are
 *   installed independently of atomic-workflows so pi's npm-identity
 ...
 *   pi's slash-command and tool registries, and tell the user exactly
 *   how to install whichever ones are missing.
 *
 ...
 *      registered by the companion (e.g. `/subagents-doctor`).
 *   3. Tool-name hints (fallback): a registered tool name (e.g. `mcp`).
 *
 * Any match counts as "installed". When nothing matches, we surface a
 * single-line `pi install npm:<spec>` hint in the doctor card.
 *
 * cross-ref:
 *  - src/extension/doctor.ts (consumes detectCompanions)
 *  - src/tui/doctor-card.ts (renders companion rows + install hints)
...
/** One entry per companion pi package atomic-workflows can use at runtime. */
export interface CompanionSpec {
  /** Display name, e.g. `pi-subagents`. */
  readonly name: string;
  /** Install spec for the doctor hint: `npm:pi-subagents`. */
...
/**
 * Ordered companion catalogue. Order is preserved in the doctor card so
 * the user sees the same layout every run — handy for diffing two
 * machines' setups.
 */
