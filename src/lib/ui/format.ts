...
/**
 * Normalize markdown block content for terminal rendering.
 *
 * OpenCode's markdown block rendering in OpenTUI trims leading/trailing
 * whitespace but preserves internal newlines so list, blockquote, and
 * paragraph formatting remains intact.
 *
...
/**
 * Join multiple thinking/reasoning blocks while preserving block boundaries.
 *
 * Streaming metadata stores per-source reasoning fragments separately. When we
 * need a single text blob (for transcript mode or message finalization), join
 * non-empty blocks with a blank line so markdown headings/paragraphs from
 * distinct reasoning blocks do not run together.
 */
export function joinThinkingBlocks(blocks: Iterable<string>): string {
  let combined = "";
