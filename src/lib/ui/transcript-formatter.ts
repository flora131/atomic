...
import { truncateText, formatTimestamp as formatTimestampFull } from "@/lib/ui/format.ts";
import { getHitlResponseRecord } from "@/lib/ui/hitl-response.ts";
import { STATUS, PROMPT, SPINNER_FRAMES, SPINNER_COMPLETE, SEPARATOR, MISC } from "@/theme/icons.ts";
...
function getThinkingBlocks(msg: ChatMessage, liveThinkingText?: string): string[] {
  const reasoningBlocks = (msg.parts ?? [])
    .flatMap((part) => {
  ...
  const fallbackThinkingContent = msg.thinkingText || (!msg.streaming ? undefined : liveThinkingText);
...
export function formatTranscript(options: FormatTranscriptOptions): TranscriptLine[] {
  ...
  for (const msg of messages) {
    if (msg.role === "user") {
      ...
      // Thinking trace (prefer per-part reasoning blocks to preserve boundaries).
      const thinkingBlocks = getThinkingBlocks(msg, liveThinkingText);
