...
/**
 * Vendored from @opentui/core (packages/core/src/lib/tree-sitter/assets/update.ts)
 * until the published package ships the runnable script.
 *
 * @see https://opentui.com/docs/reference/tree-sitter/#automated-asset-management
 */

import { readFile, writeFile, mkdir } from "fs/promises"
import * as path from "path"
import { DownloadUtils } from "./download-utils.ts"
...
function getDefaultOptions(): UpdateOptions {
  return {
    configPath: path.resolve(import.meta.dir, "../../parsers-config"),
    assetsDir: path.resolve(import.meta.dir, "../../src/parsers"),
    outputPath: path.resolve(import.meta.dir, "../../src/parsers.ts"),
  }
}
...
${queriesLines.join("\n")}
  ...
  const fileContent = `// This file is generated by src/scripts/update-parsers.ts - DO NOT EDIT MANUALLY
// Run 'bun run prebuild' to regenerate this file
...
function parseCLIArgs(): Partial<UpdateOptions> | null {
  try {
    ...
    if (values.help) {
      console.log(`Usage: bun update-parsers.ts [options]
...
Examples:
  # Use default paths
  bun src/scripts/update-parsers.ts

  # Use custom paths
  bun src/scripts/update-parsers.ts --config ./my-parsers.json --assets ./src/parsers --output ./src/parsers.ts
`)
      process.exit(0)
    }
