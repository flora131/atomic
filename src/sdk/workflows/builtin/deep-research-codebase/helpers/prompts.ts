...
export function buildHistoryAnalyzerPrompt(opts: {
  ...
  return [
    ...
    `inline path citations using absolute paths, e.g. (\`${path.join(opts.root, "research/docs/2025-…md")}\`)>`,
    ``,
...
export function buildAggregatorPrompt(opts: {
  ...
  return [
    ...
    ``,
    `## Code References`,
    `- \`<absolute path>:<line>\` — what's there (preserve the absolute paths as emitted by the explorer reports; never shorten to repo-relative)`,
    ``,
