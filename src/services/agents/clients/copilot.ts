...
import {
  loadCopilotSessionArtifacts as loadCopilotSessionArtifactsImpl,
...
export type CopilotConnectionMode = { type: "stdio" } | { type: "port"; port: number } | { type: "cliUrl"; url: string };
...
export class CopilotClient implements CodingAgentClient {
  ...
  private async listSdkModelsFromFreshClient(): Promise<unknown[]> {
    return await listCopilotSdkModelsFromFreshClient({ buildSdkOptions: () => this.buildSdkOptions(), createSdkClientInstance: (options) => this.createSdkClientInstance(options) });
  }
  ...
  setPermissionHandler(handler: CopilotPermissionHandler): void { this.permissionHandler = handler; }

  private async buildSdkOptions(): Promise<SdkClientOptions> { return await buildCopilotSdkOptions(this.clientOptions); }

  private getCopilotPermissionHandler(): CopilotPermissionHandler { return this.permissionHandler ?? createAutoApprovePermissionHandler(); }
  ...
  private wrapSession(sdkSession: SdkCopilotSession, config: SessionConfig): Session {
  ...
  private handleSdkEvent(sessionId: string, event: SdkSessionEvent): void {
    handleCopilotClientSdkEvent({
      ...
      event,
      sessions: this.sessions,
      emitMappedSdkEvent: (eventType, resolvedSessionId, data, nativeEvent, unifiedData) =>
        this.emitMappedSdkEvent(eventType, resolvedSessionId, data, nativeEvent, unifiedData),
    });
  }
  ...
  private emitMappedSdkEvent<T extends ProviderStreamEventType>(eventType: T, sessionId: string, data: ProviderStreamEventDataMap[T], nativeEvent: SdkSessionEvent, unifiedData?: Record<string, unknown>): void {
    emitMappedCopilotSdkEvent({
      eventType,
      sessionId,
      data,
  ...
  async createSession(config: SessionConfig = {}): Promise<Session> {
    return await createCopilotSession({
      ...
      config,
      sessions: this.sessions,
      loadCopilotSessionArtifacts: (projectRoot) => this.loadCopilotSessionArtifacts(projectRoot),
      listSdkModelsFresh: () => this.listSdkModelsFresh(),
      buildSdkSessionConfigBase: (sessionConfig, options) => buildCopilotClientSessionConfigBase({
  ...
  async setActiveSessionModel(model: string, options?: { reasoningEffort?: string }): Promise<void> {
    await setCopilotActiveSessionModel({
      ...
      loadCopilotSessionArtifacts: (projectRoot) => this.loadCopilotSessionArtifacts(projectRoot),
      listSdkModelsFresh: () => this.listSdkModelsFresh(),
      buildSdkSessionConfigBase: (sessionConfig, configOptions) => buildCopilotClientSessionConfigBase({
        config: sessionConfig,
  ...
  async start(): Promise<void> {
    await startCopilotRuntime({
      ...
      createSdkClientInstance: (options) => this.createSdkClientInstance(options),
      setSdkClient: (client) => { this.sdkClient = client; },
      setIsRunning: (running) => { this.isRunning = running; },
      getCopilotPermissionHandler: () => this.getCopilotPermissionHandler(),
  ...
  getState(): "disconnected" | "connecting" | "connected" | "error" { return this.sdkClient ? this.sdkClient.getState() : "disconnected"; }
  ...
  getSystemToolsTokens(): number | null { return this.probeSystemToolsBaseline; }

  getKnownAgentNames(): string[] { return this.knownAgentNames; }
}
