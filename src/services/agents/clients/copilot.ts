...
export class CopilotClient implements CodingAgentClient {
  ...
  private handleSdkEvent(sessionId: string, event: SdkSessionEvent): void {
    ...
    switch (event.type) {
      ...
      case "abort":
        return;
      default:
        // New SDK releases can add session event variants before Atomic maps them.
        // Ignore unmapped events until we add first-class handling.
        return;
    }
  }
  ...
  async setActiveSessionModel(
    ...
    let sanitizedReasoningEffort = options?.reasoningEffort;
    if (sanitizedReasoningEffort !== undefined) {
      try {
        const models = await this.listSdkModelsFresh() as CopilotSdkModelRecord[];
        const matchedModel = models.find((entry) => entry.id === resolvedModel);
        const capabilities = matchedModel?.capabilities as Record<string, unknown> | undefined;
        const supports = capabilities?.supports as Record<string, unknown> | undefined;
        if (supports?.reasoningEffort !== true) {
