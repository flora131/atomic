...
export async function sendOpenCodeSessionPrompt(args: {
	...
	if (tokens) {
		args.sessionState.inputTokens =
			tokens.input ?? args.sessionState.inputTokens;
		args.sessionState.outputTokens =
			tokens.output ?? args.sessionState.outputTokens;
	}

	const parts = result.data?.parts ?? [];
	const textParts = parts.filter((part) => part.type === "text");
	const content = textParts.map((part) => (part.text as string) ?? "").join("");
...
export async function runOpenCodeSessionCommand(args: {
	...
	const resolvedModel =
		args.runtimeArgs.getActivePromptModel() ?? args.initialPromptModel;
