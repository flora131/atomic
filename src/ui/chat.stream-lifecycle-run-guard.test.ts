...
describe("ordering diagnostics observability", () => {
  ...
  test("records post-complete delta before done projection violations", () => {
    ...
    expect(metrics.counters["workflow.runtime.parity.agent_ordering_contract_violation_total{provider=claude,scenario=single}"]).toBe(1);
  });
  ...
  test("does not record violation counters when done is already projected", () => {
    ...
    expect(metrics.counters["workflow.runtime.parity.agent_ordering_contract_violation_total{provider=claude,scenario=multi}"]).toBeUndefined();
  });
});
