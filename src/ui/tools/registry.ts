...
function getApplyPatchText(props: ToolRenderProps): string | undefined {
  const fromInput = extractApplyPatchText(props.input);
  if (fromInput) {
    return fromInput;
  }

  return extractApplyPatchText(props.output);
}
...
function extractApplyPatchText(value: unknown): string | undefined {
  if (typeof value === "string") {
    ...
    try {
      ...
      return extractApplyPatchText(parsed);
    } catch {
      return undefined;
    }
  }
...
function getApplyPatchOutputOperations(output: unknown): ApplyPatchOperation[] {
  if (typeof output === "string") {
    try {
      ...
      return getApplyPatchOutputOperations(parsed);
    } catch {
      return [];
    }
  }
...
function getApplyPatchTitle(operations: ApplyPatchOperation[]): string {
  ...
  if (operations.length === 1) {
    const onlyPath = operations[0]!.path;
    const parts = onlyPath.split("/");
    return parts[parts.length - 1] || onlyPath;
  }
