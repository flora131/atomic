...
describe("writeTelemetryState", () => {
  ...
  test("read/write round-trip preserves state", () => {
    const original: TelemetryState = {
      enabled: true,
      consentGiven: true,
      anonymousId: "round-trip-test",
      createdAt: "2026-01-15T12:00:00Z",
      rotatedAt: "2026-01-15T12:00:00Z",
    };

    writeTelemetryState(original);
...
describe("shouldRotateId", () => {
  test("returns true on month boundary (different month)", () => {
    const state: TelemetryState = {
      enabled: true,
      consentGiven: true,
      anonymousId: "test",
      createdAt: "2026-01-01T00:00:00Z",
      rotatedAt: "2025-12-15T00:00:00Z", // Last month
    };

    expect(shouldRotateId(state)).toBe(true);
  });

  test("returns true on year boundary", () => {
    const state: TelemetryState = {
      enabled: true,
      consentGiven: true,
      anonymousId: "test",
      createdAt: "2025-01-01T00:00:00Z",
      rotatedAt: "2025-12-15T00:00:00Z", // Last year
    };
...
describe("rotateAnonymousId", () => {
  test("generates new ID that differs from old", () => {
...
describe("initializeTelemetryState", () => {
  ...
  test("consentGiven defaults to false", () => {
    const state = initializeTelemetryState();
...
describe("isTelemetryEnabled", () => {
  ...
  test("returns false for ATOMIC_TELEMETRY=0", async () => {
    process.env.ATOMIC_TELEMETRY = "0";
    expect(await isTelemetryEnabled()).toBe(false);
  });

  test("returns false for ATOMIC_TELEMETRY=false", async () => {
