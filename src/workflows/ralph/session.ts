 ...
 * - Task tracking (list, current index, completed tasks)
...
export interface RalphSession {
  ...
  /** List of tasks to implement in this session */
  tasks: TodoItem[];
 ...
 *
 * @param sessionDir - Path to the session directory
 * @param entry - The progress entry string to append
 *
 * @example
 * ```typescript
 * await appendProgress(".ralph/sessions/abc123/", "Completed task: Add user authentication");
 * ```
 */
