...
// ---------------------------------------------------------------------------
// project-local: {cwd}/.omp/workflows/
// ---------------------------------------------------------------------------

describe("discoverWorkflows — project-local", () => {
  test("loads workflow from .omp/workflows/ and registers it", async () => {
    const cwd = makeTempDir("proj-local");
    const wfDir = join(cwd, ".omp", "workflows");
  ...
  test("empty .omp/workflows/ produces no sources and no errors", async () => {
    const cwd = makeTempDir("proj-local-empty");
    mkdirSync(join(cwd, ".omp", "workflows"), { recursive: true });
  ...
  test("missing .omp/workflows/ dir is silent (no error)", async () => {
...
// ---------------------------------------------------------------------------
// user-global: {homeDir}/.omp/agent/workflows/
// ---------------------------------------------------------------------------

describe("discoverWorkflows — user-global", () => {
  test("loads workflow from homeDir/.omp/agent/workflows/", async () => {
    const homeDir = makeTempDir("user-global");
    const wfDir = join(homeDir, ".omp", "agent", "workflows");
    mkdirSync(wfDir, { recursive: true });
...
describe("discoverWorkflows — DUPLICATE_NAME precedence", () => {
  ...
  test("project-local beats user-global: user-global emits DUPLICATE_NAME", async () => {
    ...
    const ugDir = join(homeDir, ".omp", "agent", "workflows");
