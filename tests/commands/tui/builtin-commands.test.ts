...
import { CommandRegistry, globalRegistry } from "@/commands/tui/registry.ts";
    ...
    describe("modelCommand", () => {
        ...
        test("returns a clear error when session bootstrap fails", async () => {
            ...
            expect(result.message).toContain(
                "Failed to start session for /model",
            );
        ...
        test("invalidates the model cache on repeated /model invocations in the same session", async () => {
        ...
        test("switches model successfully", async () => {
            ...
            expect(result.stateUpdate).toHaveProperty(
                "model",
                "claude-sonnet-4",
            );
        });
