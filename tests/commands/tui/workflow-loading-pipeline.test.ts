...
describe("importWorkflowModule", () => {
  ...
  test("bundles and imports a workflow using the programmatic Bun.build API", async () => {
    const workflowFile = join(tempDir, "simple-workflow.ts");
    await writeFile(
      workflowFile,
      `export default {
        name: "simple-workflow",
        description: "Loaded via Bun.build",
        __compiledWorkflow: true,
      };`,
    );
    ...
    expect((mod.default as { name: string }).name).toBe("simple-workflow");
  });
});
