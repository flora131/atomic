...
// ---------------------------------------------------------------------------
// Integration: workflow protection — destructive action prevention directives
// ---------------------------------------------------------------------------

const REMAIN_SESSION = `atomic-rem-${crypto.randomUUID().slice(0, 8)}`;

describe.if(tmuxAvailable)("tmux.conf: remain-on-exit keeps pane alive after process exits", () => {
  afterAll(() => {
    killSession(REMAIN_SESSION);
  });

  test("pane remains alive after a process that exits with code 0", async () => {
...
const KILLWIN_REMAIN_SESSION = `atomic-kwr-${crypto.randomUUID().slice(0, 8)}`;

describe.if(tmuxAvailable)("tmux.conf: programmatic killWindow still works with remain-on-exit", () => {
  afterAll(() => {
    killSession(KILLWIN_REMAIN_SESSION);
  });

  test("killWindow removes a window even when remain-on-exit is on", async () => {
    createSession(KILLWIN_REMAIN_SESSION, "bash", "main-win");
    createWindow(KILLWIN_REMAIN_SESSION, "to-kill", "bash");
    await Bun.sleep(300);

    // Kill the second window programmatically — this should succeed despite remain-on-exit
    killWindow(KILLWIN_REMAIN_SESSION, "to-kill");
    await Bun.sleep(200);

    const result = tmuxRun(["list-windows", "-t", KILLWIN_REMAIN_SESSION, "-F", "#{window_name}"]);
    expect(result.ok).toBe(true);
    if (result.ok) {
      const windowNames = result.stdout.split("\n").map((n) => n.trim()).filter(Boolean);
      expect(windowNames).not.toContain("to-kill");
    }
  });
});
...
const AUTONAME_SESSION = `atomic-anm-${crypto.randomUUID().slice(0, 8)}`;

describe.if(tmuxAvailable)("tmux.conf: automatic-rename off preserves window names", () => {
  afterAll(() => {
    killSession(AUTONAME_SESSION);
  });
...
const CRASH_SESSION = `atomic-crsh-${crypto.randomUUID().slice(0, 8)}`;
