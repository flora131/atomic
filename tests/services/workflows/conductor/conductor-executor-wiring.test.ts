...
describe("executeConductorWorkflow — ConductorConfig wiring", () => {
  ...
  // -----------------------------------------------------------------------
  // 6. maxIterations passthrough from options to ConductorConfig
  // -----------------------------------------------------------------------

  describe("maxIterations passthrough", () => {
    test("options.maxIterations flows through to conductor — limits traversal steps", async () => {
      // With a 3-stage linear graph but maxIterations: 1, only the first
      // stage should execute. The conductor stops after 1 graph traversal step.
      const bus = new EventBus({ validatePayloads: false });
      const receivedEvents: BusEvent[] = [];
      bus.onAll((event) => receivedEvents.push(event));
      ...
      // With maxIterations: 1, only 1 graph node should be visited
      const startEvents = receivedEvents.filter((e) => e.type === "workflow.step.start");
      expect(startEvents).toHaveLength(1);
    });

    test("options.maxIterations undefined uses default (100) — all stages execute", async () => {
      const bus = new EventBus({ validatePayloads: false });
      const receivedEvents: BusEvent[] = [];
      bus.onAll((event) => receivedEvents.push(event));
      ...
      // No maxIterations — default is 100, which is more than enough for 2 stages
      await executeConductorWorkflow(definition, "prompt", context);
    ...
    test("options.maxIterations: 2 allows exactly 2 stages in a 3-stage workflow", async () => {
      const bus = new EventBus({ validatePayloads: false });
      const receivedEvents: BusEvent[] = [];
      bus.onAll((event) => receivedEvents.push(event));
      ...
      expect(nodeIds).toEqual(["planner", "reviewer"]);
    });
  });
