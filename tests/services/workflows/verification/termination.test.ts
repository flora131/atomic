...
const { checkTermination } = await import(
  "@/services/workflows/verification/termination"
);
...
describe("checkTermination", () => {
  test("verifies a simple linear graph (all paths reach end)", async () => {
  ...
  test("detects a dead-end node (non-end node with no successors)", async () => {
    const graph = makeEncodedGraph({
      nodes: [{ id: "A" }, { id: "B" }, { id: "C" }],
      edges: [{ from: "A", to: "B" }],
      startNode: "A",
    ...
    expect(result.counterexample).toContain("no path to any end node");
    expect(result.details?.deadEndNodes).toContain("B");
  });

  test("verifies a diamond graph (all paths lead to end)", async () => {
    const graph = makeEncodedGraph({
  ...
  test("verifies a graph with multiple end nodes", async () => {
    const graph = makeEncodedGraph({
      nodes: [
        { id: "A" },
        { id: "B" },
  ...
  test("detects multiple dead-end nodes", async () => {
    const graph = makeEncodedGraph({
      nodes: [{ id: "A" }, { id: "B" }, { id: "C" }, { id: "end" }],
      edges: [
        { from: "A", to: "B" },
        { from: "A", to: "C" },
      ],
  ...
  test("verifies a graph with a cycle that has an exit to end node", async () => {
    const graph = makeEncodedGraph({
      nodes: [{ id: "A" }, { id: "B" }, { id: "C" }, { id: "D" }],
      edges: [
        { from: "A", to: "B" },
  ...
  test("detects a pure cycle with no exit (cannot terminate)", async () => {
    const graph = makeEncodedGraph({
  ...
  test("returns verified for end node that is also start node", async () => {
    const graph = makeEncodedGraph({
      nodes: [{ id: "start_end" }, { id: "helper" }],
      edges: [
        { from: "start_end", to: "helper" },
        { from: "helper", to: "start_end" },
      ],
      startNode: "start_end",
      endNodes: ["start_end"],
    });
  ...
  test("counterexample message format for dead-end nodes", async () => {
    const graph = makeEncodedGraph({
      nodes: [{ id: "start" }, { id: "deadEnd" }, { id: "end" }],
      edges: [{ from: "start", to: "deadEnd" }],
      startNode: "start",
      endNodes: ["end"],
    });
    ...
    expect(result.counterexample).toBe(
      'Node(s) "deadEnd" have no path to any end node',
    );
  });

  test("returns verified for a two-node graph", async () => {
    const graph = makeEncodedGraph({
