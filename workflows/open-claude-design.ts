...
export default defineWorkflow("open-claude-design")
  ...
  .run(async (ctx) => {
    ...
    const onboardingStage = await ctx.stage("onboarding");
    ...
    const importStage = await ctx.stage("import");
    ...
    const generateStage = await ctx.stage("generate");
    ...
    const refineStage = await ctx.stage("refine");
    ...
    const handoffStage = await ctx.stage("export-handoff");
